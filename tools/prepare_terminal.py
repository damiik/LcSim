...
def prepare(source,program,mode="apple1"):
    if mode not in ("apple1","acia"):raise ValueError("unknown terminal mode")
    ...
    rows=[dict(t='TERMINAL',n='Console',i=[nodes[n] for n in names],o=outputs,io_base=0x5000 if mode=='acia' else 0xd010,io_mode=mode,x=3000,y=1100,cd='Polling console: '+mode,nnp='CENTER')]
...
def main():
    p=argparse.ArgumentParser(description=__doc__);p.add_argument('--mode',choices=['apple1','acia'],default='apple1');p.add_argument('cpu',type=Path);p.add_argument('rom',type=Path);p.add_argument('output',type=Path);a=p.parse_args()
    a.output.write_text(prepare(a.cpu.read_text(),hex_words(a.rom.read_text()),a.mode))
