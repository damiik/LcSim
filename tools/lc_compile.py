...
def compile_file(path, output):
 ...
 normalized={};scope=[];scope_config={};net_names_by_module={}
 for key,table in tables.items():
  ...
  # NODE name-group union-find. NODE elements are schematic connectors only;
  # they carry no logic and no delay. Every pin of same-named NODEs is one
  # electrical net (union-find). Invisible NODEs (default circle + default
  # colors) are dropped from the emitted design; visible NODEs (custom shape
  # or colors, used as PANELS pixels) stay. Record net names per module.
  npar={}
  def nroot(n):
   npar.setdefault(n,n)
   if npar[n]!=n:npar[n]=nroot(npar[n])
   return npar[n]
  def nunion(a,b):
   if a and b:npar[nroot(b)]=nroot(a)
  def npins(r):
   pins=[]
   for pin in r['i']:
    if isinstance(pin,list):pins.extend([w for w in pin if w])
    elif pin:pins.append(pin)
   for pin in r['o']:
  ...
  for r in nodes:
   pins=npins(r)
   for p in pins:nroot(p)
   for p in pins[1:]:nunion(pins[0],p)
  by_name={}
  for r in nodes:
   nm=r.get('n','')
  ...
  for nm,grp in by_name.items():
   ...
   pins=[]
   for r in grp:pins.extend(npins(r))
   for p in pins[1:]:nunion(pins[0],p)
  # Rewrite every pin through the NODE roots.
  for r in rows['inputs']+rows['gates']+rows['outputs']:
   new_i=[]
   for pin in r['i']:
    if isinstance(pin,list):new_i.append([nroot(w) for w in pin])
    elif pin==0:new_i.append(0)
    else:new_i.append(nroot(pin))
   r['i']=new_i
   r['o']=[nroot(w) if w else 0 for w in r['o']]
  # Record named nets for this module (root -> first name seen).
  module_net_names={}
  for r in nodes:
   nm=r.get('n','')
   if not nm:continue
  ...
  # Drop invisible NODEs (default circle shape + default colors).
  def is_visible(r):
   return (str(r.get('nsh','circle')).lower()!='circle' or
           str(r.get('non','#a8ffaa')).lower()!='#a8ffaa' or
           str(r.get('nof','#3b351d')).lower()!='#3b351d')
  rows['gates']=[r for r in rows['gates'] if r.get('t')!='NODE' or is_visible(r)]
  normalized[key]=rows
 ...
 lines=['// Generated by LcSim. Edit the TOML source, then regenerate.','#include <lcsim/model.hpp>','namespace lc {']
 # Net-name lookup helper. Definition-local net numbers; used by debug tools
 # to recover the schematic name of a canonical net after NODE merging.
 lines+=['const char* find_net_name(int def,uint32_t net) {',' switch(def) {']
 for k in keys:
  nms=net_names_by_module.get(k,{})
  if not nms:continue
  lines+=[' case '+str(ids[k])+': {']
  lines+=['  static const std::pair<uint32_t,const char*> tbl[]={']
  for root,nm in sorted(nms.items()):
   lines+=['   {'+str(root)+','+q(nm)+'},']
  lines+=['  };','  for(auto&p:tbl)if(p.first==net)return p.second;','  return nullptr;',' }']
 lines+=[' default: return nullptr;',' }','}']
 lines+=['Design make_design() {',' Design d; d.name='+q(tables['workspace'].get('n',path.stem))+'; d.root='+str(ids['workspace'])+';',' d.definitions.resize('+str(len(keys))+');']
 for k in keys:
  ...
  wire_names=net_names_by_module.get(k,{})
  ...
  for ordinal,r in enumerate(allrows):
   ...
   if t=='TERMINAL' and r.get('io_mode','apple1') not in ['apple1','acia']:raise ValueError('terminal io_mode must be apple1 or acia')
   ...
   if module>=0:
    enum_name='MODULE'
   elif t in ('AND','OR','XOR','NAND','NOR') and len(r['i'])==2:
    enum_name=t+'2'
   else:
    enum_name={'DFF':'D_FF','SW':'SWITCH','PULL':'PULLUP'}.get(t,t)
