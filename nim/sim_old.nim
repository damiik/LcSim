...
# Non-blocking host stdin poller. Called from advance() on a wall-clock
# schedule, not per simulator tick, so a benchmark run performs a bounded
# number of syscalls regardless of step count.
proc pollTerminal(s: Simulator) =
  ...
  if r <= 0: return
  if (fds[0].revents and POLLIN) == 0: return
...
proc pure(t: GateType): bool {.inline.} =
  t in {gtAND, gtOR, gtXOR, gtNAND, gtNOR, gtNOT, gtBUF, gtTBUF, gtOC, gtMUX2}
...
proc flatten(s: Simulator, d: Design, defIdx: int,
  ...
  # NODE/INOUT aliases within this instance
  ...
  for e in m.elements:
    if e.typ in {gtNODE, gtINOUT}:
      ...
      if e.name.len > 0:
        if e.name in named: s.join(w, named[e.name])
        else: named[e.name] = w

  for e0 in m.elements:
    var e = e0
    for i in 0 ..< e.ins.len:  e.ins[i]  = map[e.ins[i].int]
    for i in 0 ..< e.outs.len:
      if e.outs[i] != 0.Net: e.outs[i] = map[e.outs[i].int]
      else:                  e.outs[i] = s.alloc()

    if e.module >= 0:
      s.flatten(d, e.module, e.ins, e.outs, false)
      continue

      # SWITCH: not a gate, a conditional wire link. In this PoC we treat
      # SWITCH as always-on when its enable input is H (static behaviour).
      # Full switch-domain resolution is out of scope for the PoC.
      if e.typ == gtSWITCH:
    ...
    if e.typ == gtSWITCH: g.activeLow = true
    for n in e.outs:
...
proc enqueue(s: Simulator, at: Time, kind: uint8, id: int32,
  ...
  for k in s.bucketHead ..< s.buckets.len:
    if s.buckets[k].at == at:
      s.buckets[k].events.add(e); return
    if s.buckets[k].at > at:
      var nb: Bucket
      nb.at = at
      nb.events = @[e]
      s.buckets.insert(nb, k); return
  var nb: Bucket
  nb.at = at
  nb.events = @[e]
  s.buckets.add(nb)
...
proc schedule(s: Simulator, id: int32, v: Logic, dt: Time, weak = false) =
  let d = addr s.drivers[id.int]
  if d.pending and d.target == v and d.targetWeak == weak: return
  if not d.pending and d.value == v and d.weak == weak: return
  d.generation.inc
  d.pending = false
  d.target = v
  d.targetWeak = weak
  if d.value == v and d.weak == weak: return
  d.pending = true
...
proc resolve(s: Simulator) =
  ...
  for n in s.touchedList:
    ...
    if w.drivers.len == 1:
      ...
      if d.value == w.value and d.weak == w.weak: continue
      w.value = d.value
      w.weak = d.weak
      for u in w.users: s.markDirty(u)
      continue
    var strong = lZ
    var weak = lZ
    for id in w.drivers:
      let d = s.drivers[id.int]
      if d.value == lZ: continue
      if d.weak:
        if weak == lZ: weak = d.value
        elif weak != d.value: weak = lX
      else:
        if strong == lZ: strong = d.value
        elif strong != d.value: strong = lX
    let v = if strong == lZ: weak else: strong
    let isWeak = strong == lZ
    if v == w.value and isWeak == w.weak: continue
    w.value = v
    w.weak = isWeak
    for u in w.users: s.markDirty(u)

  for n in s.touchedList: s.touchedFlag[n.int] = 0
  s.touchedList.setLen(0)

# ---------- gate evaluation ----------

proc delayFor(s: Simulator, g: Gate): Time =
  if g.e.cycles >= 0: return Time(g.e.cycles * g.period.float + 0.5)  # not used; period=0 default
  if g.e.ns >= 0:     return Time(g.e.ns * 1000.0 + 0.5)
  case g.e.typ
  of gtDFF: return s.profile.cq
  of gtSWITCH: return s.profile.sw
  else: return s.profile.gate
  ...
  proc evaluatePure(s: Simulator, id: int32) =
    ...
    let nin = g.e.ins.len
    for i in 0 ..< nin: v[i] = s.nets[g.e.ins[i].int].value
    ...
    of gtXOR:
      ...
      for i in 0 ..< nin:
        let x = binary(v[i])
        if x == lX or r == lX: r = lX
        elif x == lH: r = inv(r)
      outs[0] = r
    of gtMUX2, gtMUX4, gtDMUX2, gtDMUX4:
      # Layout follows the C++ reference:
      #   MUX2  : [d0, d1, sel]
      #   MUX4  : [d0, d1, d2, d3, sel0, sel1]
      #   DMUX2 : [data, sel]
      #   DMUX4 : [data, sel0, sel1]
      # Unknown select bits (X) fan out to every candidate lane; unequal
      # resulting outputs collapse to X.
      for i in 0 ..< nin:
        if v[i] == lZ: v[i] = lX
      ...
      for p in 0 ..< nout: outs[p] = lX
      var first = true
      for c in 0 ..< count:
        if v[offset] != lX and ord(v[offset]) != (c and 1): continue
        if count == 4 and v[offset + 1] != lX and
           ord(v[offset + 1]) != (c shr 1): continue
        for p in 0 ..< nout:
          let x: Logic =
            if dem: (if p == c: v[0] else: lL)
            else:   v[c]
          if first or outs[p] == x: outs[p] = x
          else: outs[p] = lX
        first = false
    else:
      discard
...
proc evaluateStateful(s: Simulator, id: int32) =
  ...
  of gtDFF:
    let clk = binary(v[0])
    let data = binary(v[1])
    let setupD = if g.e.setup >= 0: Time(g.e.setup * 1000 + 0.5) else: s.profile.setup
    let holdD = if g.e.hold >= 0: Time(g.e.hold * 1000 + 0.5) else: s.profile.hold
    if data != g.lastData:
      g.dataChanged = s.now
      if g.edge >= 0 and s.now - g.edge < holdD: g.stored = lX
      g.lastData = data
    if not s.initializing and clk == lH and g.lastClock != lH:
      g.edge = s.now
      g.stored = if s.now - g.dataChanged < setupD: lX else: data
    g.lastClock = clk
  ...
  of gtDLATCH:
    ...
    let data = binary(v[0])
    if v[0] != lZ:
      if en == lH: g.stored = data
      elif en == lX and data != g.stored: g.stored = lX
    s.schedule(drvBegin.int32, g.stored, dt)
    s.schedule(drvBegin.int32 + 1, inv(g.stored), dt)
  ...
  of gtTERMINAL:
    ...
    #   in[24]       = /RD   (active low)
    #   in[25]       = /WR   (active low)
    # ioBase selects a 4-byte register window.
    #   reg 0 read : keyboard byte, bit7=1 when a fresh key is present
    #   reg 1 read : status, bit1=1 (TX ready), always set
    #   reg 1 write: emit byte to host stdout
    # Register 0 write is a no-op (Apple1 has no control register).
    var a: uint64 = 0
    var aValid = true
    for b in 0 ..< 16:
      let x = v[b]
      if x != lL and x != lH: aValid = false; break
      if x == lH: a = a or (1'u64 shl b)
    if aValid:
      ...
      if a >= base and a < base + 4:
        ...
        if wr and reg == 1:
          var d: uint64 = 0
          for b in 0 ..< 8:
            if v[16 + b] == lH: d = d or (1'u64 shl b)
  ...
  of gtROM, gtRAM:
    ...
    for b in 0 ..< g.e.aw.int:
      let x = v[b]
      if x != lL and x != lH: valid = false; break
      if x == lH: memAddr = memAddr or (1'i64 shl b)
    if not valid: memAddr = -1
    var changed = false
    if t == gtRAM:
      ...
      if we == lL:
        ...
        for b in 0 ..< g.e.dw.int:
          ...
          if x != lL and x != lH: wvalid = false; break
          if x == lH: word = word or (1'u64 shl b)
        if wvalid:
          let i = memAddr.int
    ...
    if memAddr != g.address:
      ...
      if g.memoryDelay == 0:
        ...
        g.readWord = if memAddr >= 0: g.e.memory[memAddr.int] else: 0
      else:
        g.readGeneration.inc
        s.enqueue(s.now + g.memoryDelay, 2'u8, id, lX, g.readGeneration, false)
    ...
    for b in 0 ..< g.e.dw.int:
      var outV = lZ
      if oe == lL:
        if not g.ready: outV = lZ
        elif g.settledAddress < 0: outV = lX
        else: outV = if ((g.readWord shr b) and 1) == 1: lH else: lL
      elif oe != lH: outV = lX
...
proc evaluateDirty(s: Simulator) =
  if s.dirtyGates.len == 0: return
  for g in s.dirtyGates:
    if pure(s.gates[g.int].e.typ): s.evaluatePure(g)
    else: s.evaluateStateful(g)
  for g in s.dirtyGates: s.dirty[g.int] = false
  s.dirtyGates.setLen(0)

# ---------- main loop ----------

proc advance*(s: Simulator, duration: Time) =
  # Poll host stdin at most every pollInterval wall-seconds. Bounded number
  # of syscalls per run, independent of simulator step count.
  let wall = epochTime()
  ...
  var prev: Time = -1
  s.evaluateDirty()
  while s.bucketHead < s.buckets.len and s.buckets[s.bucketHead].at <= stopAt:
    let t = s.buckets[s.bucketHead].at
    s.now = t
    if t != prev: sameTime = 0; prev = t
    var changed = false
    var i = 0
    while i < s.buckets[s.bucketHead].events.len:
      let e = s.buckets[s.bucketHead].events[i]; inc i
      sameTime.inc
        ...
        if s.autoClock:
          if g.fastClock:
            ...
            if s.nets[n.int].value != nv or s.nets[n.int].weak:
              ...
              for u in s.nets[n.int].users: s.markDirty(u)
          else:
            s.schedule(g.driverBegin, inv(s.drivers[g.driverBegin.int].value), 0)
        s.enqueue(s.now + g.period, 1'u8, e.id, lX, 0, false)
      of 2'u8:
        let g = s.gates[e.id.int].addr
        if e.generation != g.readGeneration: continue
        g.ready = true
        g.settledAddress = g.address
        if g.address >= 0: g.readWord = g.e.memory[g.address.int]
        s.markDirty(e.id)
      else: discard
    s.buckets[s.bucketHead].events.setLen(0)
    s.bucketHead.inc
    if changed: s.resolve()
    s.evaluateDirty()
  if s.bucketHead > 64 and s.bucketHead * 2 > s.buckets.len:
    s.buckets.delete(0, s.bucketHead - 1)
    s.bucketHead = 0
  s.now = stopAt

# ---------- construction ----------

proc newSimulator*(d: Design, prof: Profile, useCache = false): Simulator =
  new(result)
  result.profile = prof
  ...
  result.pollInterval = 0.005   # 5 ms
  discard result.alloc()   # net 0 = ground / undriven
  result.flatten(d, d.root, @[], @[], true)

  result.touchedFlag = newSeq[uint8](result.nets.len)
  result.touched.reserve(256)
  result.touchedList.reserve(256)
  result.buckets.reserve(32)

  # remap drivers/users
  for i in 0 ..< result.gates.len:
    let g = result.gates[i].addr
    for j in 0 ..< g.e.ins.len:  g.e.ins[j]  = result.root(g.e.ins[j])
    for j in 0 ..< g.e.outs.len: g.e.outs[j] = result.root(g.e.outs[j])
  ...
  for i in 0 ..< result.nets.len:
    ...
    for u in result.nets[i].users:
      if not seen.containsOrIncl(u): uniq.add(u)
    result.nets[i].users = uniq

  # precompute delays & fast clock
  for i in 0 ..< result.gates.len:
    let g = result.gates[i].addr
    g.delay = result.delayFor(g[])
    case g.e.typ
    of gtCLK: g.period = Time(result.profile.tick) * Time(g.e.period)
    of gtROM, gtRAM: g.memoryDelay = Time(g.e.memoryNs * 1000.0 + 0.5)
    else: discard
  for i in 0 ..< result.gates.len:
    let g = result.gates[i].addr
    if g.e.typ != gtCLK: continue
    if g.driverCount == 0: continue
    let dr = g.driverBegin.int
    let n = result.drivers[dr].net
    if result.nets[n.int].drivers.len == 1: g.fastClock = true

  result.dirty = newSeq[bool](result.gates.len)
  result.dirtyGates.reserve(result.gates.len)
  for i in 0 ..< result.gates.len: result.markDirty(i.int32)

  # drive initial values from sources
  for i in 0 ..< result.gates.len:
    let g = result.gates[i].addr
    let t = g.e.typ
    if t in {gtINPUT, gtCLK, gtH, gtL, gtPULLUP, gtPULLDOWN}:
      let v = if t in {gtINPUT, gtCLK, gtH, gtPULLUP}: lH else: lL
      let isWeak = t in {gtPULLUP, gtPULLDOWN}
      for d in 0 ..< g.driverCount:
        result.schedule(g.driverBegin + int32(d), v, 0, isWeak)

  result.advance(0)
  var guard = 0
  while result.bucketHead < result.buckets.len:
    guard.inc
    if guard > 10000: raise newException(ValueError, "startup did not settle")
    let nextAt = result.buckets[result.bucketHead].at
    if nextAt > 100_000_000: raise newException(ValueError, "startup >100us")
