...
namespace lc  {
  namespace  {
    ...
    Shader crt_shader{};
    bool crt_shader_loaded=false;
    int crt_loc_size=-1,crt_loc_radius=-1,crt_loc_curve=-1;
    RenderTexture2D crt_target{};
    bool crt_target_ready=false;
    int crt_target_w=0,crt_target_h=0;
    void load_crt_shader()  {
      const char* shader_paths[]={"assets/shaders/crt_terminal.fs","../assets/shaders/crt_terminal.fs"};
      for(const auto* path:shader_paths)if(FileExists(path))  {
        crt_shader=LoadShader(nullptr,path);
        crt_shader_loaded=crt_shader.id!=0;
        if(crt_shader_loaded)  {
          crt_loc_size=GetShaderLocation(crt_shader,"uSize");
          crt_loc_radius=GetShaderLocation(crt_shader,"uRadius");
          crt_loc_curve=GetShaderLocation(crt_shader,"uCurve");
        }
        break;
      }
    }
    void unload_crt_resources()  {
      if(crt_target_ready)  {
        UnloadRenderTexture(crt_target);
        crt_target_ready=false;
      }
      if(crt_shader_loaded)  {
        UnloadShader(crt_shader);
        crt_shader_loaded=false;
      }
    }
...
void draw_terminal(Simulator& sim,Rectangle area,int& scroll,std::string& status)  {
  ...
  int tw=std::max(1,int(screen.width)),th=std::max(1,int(screen.height));
  if(!crt_target_ready||crt_target_w!=tw||crt_target_h!=th)  {
    if(crt_target_ready)UnloadRenderTexture(crt_target);
    crt_target=LoadRenderTexture(tw,th);
    crt_target_w=tw;crt_target_h=th;
    crt_target_ready=true;
  }
  ...
  float ref=std::max(1.f,MeasureTextEx(font,"M",64.f,1.0f).x);
  ...
  //float x0=(screen.width-pitch*cols)/2;
  float x0=14;
  //float pitch=cell_w;      // komórki wypełniają całą szerokość
  float y0=(screen.height-cell_h*trows)/2;
  ...
  ClearBackground({8,14,12,255});
  DrawRectangleLinesEx({0,0,float(tw),float(th)},1,line);
  ...
  if(int(GetTime()*2.5)%2==0)  {                      // cursor on the SAME grid
    ...
    float cy=y0+2+float(terminal.cursor_y)*cell_h;
    DrawRectangleRec({cx,cy,pitch-4,cell_h-8},phosphor);
  ...
  if(terminal.crt)  {
    constexpr float period=4.f;   // soft, dark, soft, gap — repeats every 4px
    for(float sy=0;sy+2<float(th);sy+=period)  {
      DrawRectangleRec({0,sy,float(tw),1},{0,0,0,70});    // intermediate line above
      DrawRectangleRec({0,sy+1,float(tw),1},{0,0,0,125}); // the dark scanline
      DrawRectangleRec({0,sy+2,float(tw),1},{0,0,0,70});  // intermediate line below
      // sy+3 left untouched: a clear gap so lines read as distinct bands
    }
  }
  EndTextureMode();

  if(terminal.crt&&crt_shader_loaded)  {
    float dims[2]={float(tw),float(th)};
    float radius=18.f,curve=0.055f;
    SetShaderValue(crt_shader,crt_loc_size,dims,SHADER_UNIFORM_VEC2);
    SetShaderValue(crt_shader,crt_loc_radius,&radius,SHADER_UNIFORM_FLOAT);
    SetShaderValue(crt_shader,crt_loc_curve,&curve,SHADER_UNIFORM_FLOAT);
    BeginShaderMode(crt_shader);
    // explicit source+dest rects (both positive): position/size no longer depend on
    // the negative-height "flip" trick, which was landing the quad at the wrong spot
    // when combined with the custom shader. The vertical flip now happens in the shader.
    DrawTexturePro(crt_target.texture,{0,0,float(tw),float(th)},{screen.x,screen.y,float(tw),float(th)},{0,0},0.f,WHITE);
    EndShaderMode();
  }
  else DrawTextureRec(crt_target.texture,{0,0,float(tw),-float(th)},{screen.x,screen.y},WHITE);
}
